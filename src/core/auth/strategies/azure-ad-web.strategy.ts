...
@Injectable()
export class AzureADStrategyWeb extends PassportStrategy(
  OIDCStrategy,
  'azure-ad-web',
) {
  constructor(private usersService: UsersService, config: ConfigService) {
    super(<IOIDCStrategyOption>{
      ...
      redirectUrl:
        config.get('AUTH_AD_REDIRECT_DOMAIN') + '/auth/microsoft/connect-web',
      allowHttpForRedirectUrl: true, //process.env.NODE_ENV !== 'production',
      responseMode: 'query',
